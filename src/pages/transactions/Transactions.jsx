...
import { FaPlus, FaTrash, FaEdit, FaSearch, FaFilter, FaFileExcel, FaFilePdf } from 'react-icons/fa';
  ...
  const SummaryCard = ({ title, amount, type }) => (
    <div className="glass p-4 rounded-2xl border border-slate-800 flex-1">
      ...
      <p className={`text-xl font-bold mt-1 ${
        ...
        type === 'expense' ? 'text-rose-400' : 'text-indigo-400'
      }`}>
          ...
          {filteredTransactions.map((t) => (
            ...
            >
              ...
              <div className="text-right">
                <p className={`text-sm font-bold mb-2 ${t.type === 'income' ? 'text-emerald-400' : 'text-rose-400'}`}>
                  {t.type === 'income' ? '+' : '-'}৳{parseFloat(t.amount).toLocaleString()}
                </p>
                <div className="flex justify-end gap-1">
                  <button onClick={() => handleOpenEditModal(t)} className="p-2 text-slate-500 hover:text-indigo-400 bg-slate-800/50 rounded-lg">
                    <FaEdit size={12} />
                  </button>
                  <button onClick={() => handleDelete(t.id)} className="p-2 text-slate-500 hover:text-rose-400 bg-slate-800/50 rounded-lg">
                    <FaTrash size={12} />
                  </button>
                </div>
              </div>
              ...
              <AnimatePresence>
                {filteredTransactions.map((t) => (
                  ...
                  >
                    ...
                    <td className={`px-6 py-4 font-bold ${t.type === 'income' ? 'text-emerald-400' : 'text-rose-400'}`}>
                      {t.type === 'income' ? '+' : '-'}৳{parseFloat(t.amount).toLocaleString()}
                    </td>
